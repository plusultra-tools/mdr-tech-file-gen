...
Loads the Jinja2 template and renders the MDR Annex II technical documentation
skeleton from a validated :class:`~mdr_techfile.spec.DeviceSpec`.
"""
...
_ANNEX2_TEMPLATE = "annex-II.md.j2"
_STANDARDS_TEMPLATE = "standards-matrix.md.j2"
...
def render_annex2(
    ...
    """Render the Annex II Markdown skeleton from a device spec.
    ...
    env = _jinja_env()
    tmpl = env.get_template(_ANNEX2_TEMPLATE)
    return tmpl.render(
        spec=spec,
        pointers=filtered,
        generated_at=ts,
...
def render_standards_matrix(
    spec: DeviceSpec,
    timestamp_iso: str | None = None,
) -> str:
    """Render the standards citation matrix Markdown from a device spec.
    ...
    ts = timestamp_iso or datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
    env = _jinja_env()
    tmpl = env.get_template(_STANDARDS_TEMPLATE)
    return tmpl.render(
        spec=spec,
        all_pointers=filtered,
        generated_at=ts,
        tool_version=__version__,
    )
